...
public class ReportSalesByPaymentMethodsGenerator extends AbstractFVDialog {
   ...
   public ReportSalesByPaymentMethodsGenerator(Shell parentShell) {
      super(parentShell);
   }
   ...
   private void initFileName() {
      ...
      dlg.setFileName("Ventas_por_medios_de_pago_" + this.createDateToFile());
      String fn = dlg.open();
      if (fn != null) {
         this.setFileName(fn);
      }
   ...
   private void runThread() {
      if (!"".equals(this.getFileName())) {
         (new ReportGenerator(this.getShell().getDisplay(), this.progressBar, this.getStartDate(), this.getEndDate(), this.getOrderService(), this.getFileName(), this.lblProgressBarTitle, this.getButton(1), this)).start();
      } else {
         this.close();
      }
