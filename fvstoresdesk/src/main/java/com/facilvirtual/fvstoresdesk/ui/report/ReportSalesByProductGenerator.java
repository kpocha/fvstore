...
public class ReportSalesByProductGenerator extends AbstractFVDialog {
   ...
   private void initFileName() {
      ...
      dlg.setFileName("Ventas_por_artículos_" + this.createDateToFile());
      String fn = dlg.open();
      if (fn != null) {
         this.setFileName(fn);
      }
   ...
   private void runThread() {
      if (!"".equals(this.getFileName())) {
         (new ReportSalesByProductGeneratorInner(this.getShell().getDisplay(), this.progressBar, this.getStartDate(), this.getEndDate(), this.getOrderService(), this.getFileName(), this.lblProgressBarTitle, this.getButton(1), this)).start();
      } else {
         this.close();
      }
