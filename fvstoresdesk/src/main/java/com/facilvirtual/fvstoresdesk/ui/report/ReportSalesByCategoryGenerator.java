...
public class ReportSalesByCategoryGenerator extends AbstractFVDialog {
   ...
   public ReportSalesByCategoryGenerator(Shell parentShell) {
      super(parentShell);
   }
   ...
   private void initFileName() {
      ...
      dlg.setFileName("Ventas_por_rubros_" + this.createDateToFile());
      String fn = dlg.open();
      if (fn != null) {
         this.setFileName(fn);
      }
   ...
   private void runThread() {
      if (!"".equals(this.getFileName())) {
         (new ReportSalesByCategoryGeneratorInner(this.getShell().getDisplay(), this.progressBar, this.getStartDate(), this.getEndDate(), this.getOrderService(), this.getFileName(), this.lblProgressBarTitle, this.getButton(1), this)).start();
      } else {
         this.close();
      }
