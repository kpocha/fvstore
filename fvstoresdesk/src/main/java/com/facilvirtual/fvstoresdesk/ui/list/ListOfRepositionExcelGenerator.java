...
public class ListOfRepositionExcelGenerator extends AbstractFVDialog {
   ...
   public ListOfRepositionExcelGenerator(Shell parentShell) {
      super(parentShell);
   }
   @Override
   protected Control createDialogArea(Composite parent) {
      ...
      this.lblProgressBarTitle.setText("Generando lista de reposición...");
      this.setProducts(this.getProductService().getProductsForListOfReposition());
   ...
   private void initFileName() {
      ...
      dlg.setFileName("Lista_de_reposicion_" + this.createDateToFile());
      String fn = dlg.open();
      if (fn != null) {
         this.setFileName(fn);
      }
   ...
   @Override protected void configureShell(Shell newShell) {
      ...
      this.initTitle(newShell, "Generar lista de reposición");
   }
   @Override
   ...
   private void runThread() {
      if (!"".equals(this.getFileName())) {
         (new ListOfRepositionGenerator(this.getShell().getDisplay(), this.progressBar, this.getProducts(), this.getFileName(), this.lblProgressBarTitle, this.getButton(1), this)).start();
      } else {
         this.close();
      }
