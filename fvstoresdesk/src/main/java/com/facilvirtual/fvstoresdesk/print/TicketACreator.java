...
public class TicketACreator {
   protected static Logger logger = LoggerFactory.getLogger("TicketACreator");
   private Order order;
   ...
   public TicketACreator(Order order, AppConfig appConfig, WorkstationConfig workstationConfig, Shell shell) {
      this.order = order;
      ...
      try {
         ...
         try {
            if (this.appConfig.isCompanyLogoAvailable()) {
               int logoW = (int)this.getPoints(57);
               int logoH = (int)this.getPoints(19);
         ...
         if (!foundLogo && !"".equals(this.getAppConfig().getCompanyBusinessName().trim())) {
            grid.add(new TextPrint(this.getAppConfig().getCompanyBusinessName().trim().toUpperCase(), fontDataTitle), -1, 16777216);
         }

         if (!"".equals(this.getAppConfig().getCompanyAddressLine1ToPrint(40).trim())) {
            grid.add(new TextPrint(this.getAppConfig().getCompanyAddressLine1ToPrint(40).trim(), fontDataCommon), -1, 16777216);
         }

         if (!"".equals(this.getAppConfig().getCompanyAddressLine2ToPrint(40).trim())) {
            grid.add(new TextPrint(this.getAppConfig().getCompanyAddressLine2ToPrint(40).trim(), fontDataCommon), -1, 16777216);
         }

         if (!"".equals(this.getAppConfig().getCompanyPhoneForTicket().trim())) {
            grid.add(new TextPrint(this.getAppConfig().getCompanyPhoneForTicket().trim(), fontDataCommon), -1, 16777216);
         }
         ...
         try {
            if (!this.order.getReceiptType().getName().equalsIgnoreCase("Ticket No Fiscal")) {
               ...
               if (!"".equals(this.getAppConfig().getCompanyNameToPrint(40).trim())) {
                  grid.add(new TextPrint(this.appConfig.getCompanyNameToPrint(40).trim().toUpperCase(), fontDataCommon), -1, 16384);
               }
         ...
         grid.add(new TextPrint(" ", fontDataBr), -1, 16777216);
         if (this.order.getReceiptType().getName().equalsIgnoreCase("Ticket Factura A")) {
            grid.add(new TextPrint("Tique Factura \"A\"", fontDataCommonBold));
         ...
         if (this.order.getCustomer() != null && this.order.getCustomer().getId() != 1L) {
            try {
               grid.add(new TextPrint(" ", fontDataBr), -1, 16777216);
               if (!"".equals(this.order.getCustomer().getCompanyName().trim())) {
                  grid.add(new TextPrint(this.order.getCustomer().getCompanyName().trim().toUpperCase(), fontDataCommon), -1, 16384);
               }

               try {
                  if (!this.order.getReceiptType().getName().equalsIgnoreCase("Ticket No Fiscal")) {
                     if (this.order.hasAfipCae()) {
                        grid.add(new TextPrint("CUIT: " + this.order.getAfipDocNroToPrint(), fontDataCommon), -1, 16384);
                     } else {
                        grid.add(new TextPrint("CUIT: " + this.order.getCustomer().getCuitToPrint(), fontDataCommon), -1, 16384);
                     }

                     grid.add(new TextPrint("IVA RESPONSABLE INSCRIPTO", fontDataCommon), -1, 16384);
                  }
         ...
         grid.add(new TextPrint("(%IVA)", fontDataCommon));
         grid.add(new TextPrint("Importe", fontDataCommon));
         grid.add(new TextPrint(" ", fontDataBr), -1, 16777216);
         List<OrderLine> orderLines = this.getOrderService().getOrderLinesForOrder(this.getOrder());
         Iterator var30 = orderLines.iterator();

         while(var30.hasNext()) {
            OrderLine orderLine = (OrderLine)var30.next();
            if (orderLine.getQty() == 1.0) {
               if (this.getWorkstationConfig().isPrintCodeInTickets()) {
                  grid.add(new TextPrint(orderLine.getDescriptionWithCodeToPrint(8), fontDataCommon));
               } else {
                  grid.add(new TextPrint(orderLine.getDescriptionToPrint(15), fontDataCommon));
               }

               grid.add(new TextPrint("(" + orderLine.getVatValueToPrint() + ")", fontDataCommon));
               grid.add(new TextPrint(orderLine.getNetSubtotalToPrint(), fontDataCommon));
            } else {
               grid.add(new TextPrint(orderLine.getQtyDescriptionForTicketA(), fontDataCommon), -1, 16384);
               if (this.getWorkstationConfig().isPrintCodeInTickets()) {
                  grid.add(new TextPrint(orderLine.getDescriptionWithCodeToPrint(8), fontDataCommon));
               } else {
                  grid.add(new TextPrint(orderLine.getDescriptionToPrint(15), fontDataCommon));
         ...
         try {
            if (this.getOrder().getDiscountSurcharge() != 0.0) {
               grid.add(new TextPrint("Dto./Recargo", fontDataCommon));
               grid.add(new TextPrint("(21,00)", fontDataCommon));
               grid.add(new TextPrint(this.getOrder().getNetoDtoRecargoToDisplay(), fontDataCommon));
            }
         } catch (Exception var20) {
         }

         grid.add(new TextPrint(" ", fontDataCommon), -1, 16777216);
         grid.add(new TextPrint("SUBTOTAL", fontDataCommon));
         grid.add(new TextPrint("", fontDataCommon));
         grid.add(new TextPrint(this.order.getImpNetoToPrint(), fontDataCommon));
         grid.add(new TextPrint("IVA 21,00 %", fontDataCommon));
         grid.add(new TextPrint("", fontDataCommon));
         grid.add(new TextPrint(this.order.getImporteIVA21ToPrint(), fontDataCommon));
         grid.add(new TextPrint("IVA 10,50 %", fontDataCommon));
         grid.add(new TextPrint("", fontDataCommon));
         grid.add(new TextPrint(this.order.getImporteIVA105ToPrint(), fontDataCommon));
         grid.add(new TextPrint("CONCEPTOS NO GRAVADOS", fontDataCommon), 2, 16384);
         grid.add(new TextPrint("0,00", fontDataCommon));
         grid.add(new TextPrint("TOTAL  $", fontDataTitle));
         grid.add(new TextPrint("", fontDataCommon));
         grid.add(new TextPrint(this.order.getTotalToPrint(), fontDataTitle));
         ...
         logger.error("Message:" + var25.getMessage());
         logger.error("Exception:" + var25);
